...
3. 点击「加载已解压的扩展程序」
4. 选择 `extension/` 目录
5. 剪藏网页时保持 LLM Wiki 桌面应用运行；扩展会通过本地剪藏 API 与应用通信。

### 使用 crawl4ai 剪藏 Blog / Article

Blog 和文章剪藏使用由 LLM Wiki 应用启动的本地 `crawl4ai` helper。通过 Chrome 扩展排队 Blog URL 之前，先安装一次 Python helper 依赖：
...
- 在 `~/.llm-wiki/crawl4ai-venv` 创建本地虚拟环境
- 安装 `crawl4ai`
- 运行 `crawl4ai-setup`
- 安装 Playwright Chromium，用于浏览器式网页提取

安装完成后，重启 LLM Wiki。应用启动时会自动使用这个虚拟环境，并在 `127.0.0.1:19828` 启动 helper。

可选环境变量覆盖：

```bash
# 修改 setup 脚本创建虚拟环境的位置
export LLM_WIKI_CRAWL4AI_VENV="$HOME/.llm-wiki/crawl4ai-venv"

# 指定创建虚拟环境时使用的 Python
export LLM_WIKI_PYTHON="/path/to/python3.12"

# 指定应用启动 crawl4ai helper 时使用的 Python
...
- `crawl4ai helper is not running`：运行 `bash scripts/setup-crawl4ai-helper.sh`，然后重启 LLM Wiki。
- Chromium 或浏览器启动错误：重新运行 `bash scripts/setup-crawl4ai-helper.sh`，让脚本重新安装 Playwright Chromium。
- Chrome 扩展剪藏失败：确认 LLM Wiki 桌面应用正在运行，因为扩展需要把内容发送到应用的本地 API。
